...
function App() {
  ...
  return (
    <div className="App">
       {moviedetails.map((nm) => (
        <Movie name={nm.moviename} pic={nm.pic} summary={nm.summary} rating={nm.rating} />
      ))}
...
export default App;
function Movie({ pic, name,summary,rating }) {
  return (
    <div>
      
      <img src={pic} alt={name}></img>
      <h1>{name} <span>⭐{rating}</span></h1>
      <div className="summary"><p>Summary:-</p>{summary}</div>
